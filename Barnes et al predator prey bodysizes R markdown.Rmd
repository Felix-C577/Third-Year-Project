...
```{r}
log10fit=lm(log10(pred_mass_g)~log10(prey_mass_g),data=pred_prey_data)
summary(log10fit)
```

Reading off the summary, we see that the Intercept value = 2.921741, and the log10(prey_mass_g) intercept is 0.656835. This suggests log10(Predator Mass) = log10(2.921741) + 0.656835log10(Prey Mass). 
Therefore if this is correct then the power law relationship would be 
Predator Mass = 2.921741(Prey Mass)^0.656835

We can add this line to our logarithmic graph (in red), as well as a 1:1 line (in black) for visual purposes.
```{r}
pred_prey_data %>%
  ggplot(aes(x= log10(prey_mass_g), y= log10(pred_mass_g))) +
  geom_point(shape=20) +
  ggtitle("Log-Log Plot of Predator-Prey Masses (Base 10 logarithms)") +
  xlab("log10(Prey Mass (g))") +
  ylab("log10(Predator Mass (g))") +
  geom_abline(aes(slope = 1,intercept = 0)) +
  geom_abline(aes(slope = 0.656835,intercept = 2.921741, color="red"), show.legend=FALSE) 
...
The metadata file for the 2008 Barnes et al. study that this dataset is from states that about 99.4% of data is above the 1:1 line (or below if plotting prey mass on y axis, predator mass on x axis). We can see on the graph above that almost all of our datapoints are above the black 1:1 line so follow this relationship. This is to be expected - the mass of the predator is almost always greater than the mass of the prey.
