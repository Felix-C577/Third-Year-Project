...
Firstly, I had to download the text file containing the data from the Barnes et al study. Then I converted it to a .csv file in Excel. In this code, we import the libraries tidyverse and lubridate in case we want to use them in future, for example to use ggplot or standardise dates. Then we import the data file into R and save it to the pred_prey_data dataframe.
```{r, eval=FALSE}
library(tidyverse)
library(lubridate) 
```
...
Now if we wanted to compare the mass of a predator with the mass of its prey, we could do this using the plot function:  
```{r}
...
title(main = "Log Plot of Predator-Prey Masses" )
```

As we can see from this graph, there appears to be a positive correlation between the log of predator mass and the log of prey mass.
